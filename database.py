...
class Archive(object):
	...
	def get_data(self, age=0, scheduled_only=False):
		...
		if age <= 0:
			if scheduled_only:
				sqlCmd = 'SELECT * FROM (SELECT * FROM pi2o WHERE wxAdjust >= 0.0 OR wxAdjust <= -1.5 ORDER BY dateTimeStart DESC) AS sub GROUP BY zone LIMIT %i' % self.nZones
			else:
				sqlCmd = 'SELECT * FROM (SELECT * FROM pi2o ORDER BY dateTimeStart DESC) AS sub GROUP BY zone LIMIT %i' % self.nZones
